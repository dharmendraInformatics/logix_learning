      ...
      <div class="breadcromb-area">
         <div class="row">
            ...
            <div class="col-md-10 col-sm-10">
              <a href="<?php echo site_url('Fees/add_school_fees') ?>" class="btn btn-info pull-right">Add School Fees</a>
            </div>
         </div>
      </div>
   </div>
        ...
        <table id="my_tbl" class="my_custom_datatable table table-striped table-bordered">
          ...
          if(!empty($all_value)){
            foreach($all_value as $value){
              $edit_link = site_url('Fees/edit_school_fees?id=');
              
              if(!empty($value->m_fees_pdf)){
                $image_link = base_url('uploads/fees/').$value->m_fees_pdf;
              }
              else{
            ...
            <tr>
              ...
              <td title="Title"><?php echo $value->m_fees_title; ?></td>
              <td title="Date"><?php echo date('d-m-Y',strtotime($value->m_fees_date));?></td>
              <td title="PDF"><a href="<?php echo $image_link;?>" class="btn btn-danger" target="_blank"><i class="fa fa-file-pdf-o"></i> PDF</a></td>
              <td title="Description">
                <input type="hidden" id="m_cal_desc<?php echo $value->m_fees_id;?>" value="<?php echo $value->m_fees_desc;?>">
                <button class="btn btn-info" onclick="viewDetails(<?php echo $value->m_fees_id;?>)"><i class="fa fa-eye"></i> Description</button>
              </td>
              <td title="" style="white-space: nowrap;">
              <a href="<?php echo $edit_link.$value->m_fees_id ; ?>" class="btn btn-success btn-action" title="Edit" data-toggle="tooltip"><i class="fa fa-pencil"></i></a> 
              <button class="btn btn-danger btn-action delete-data" data-value="<?php echo $value->m_fees_id ; ?>" title="Delete" data-toggle="tooltip"><i class="fa fa-trash"></i></button>
              </td>
            </tr>
            <?php
            $i++;
...
<!-- ========================Script========================== -->
<?php $this->view('js/school_fees_js'); $this->view('js/custom_js');?>
<!-- ========================Script========================== -->