      ...
      <div class="breadcromb-area">
         <div class="row">
            ...
            <div class="col-md-10 col-sm-10">
              <a href="<?php echo site_url('Events/add_video') ?>" class="btn btn-info pull-right">Add Video</a>
            </div>
         </div>
      </div>
   </div>
        ...
        <table id="my_tbl" class="my_custom_datatable table table-striped table-bordered">
          ...
          if(!empty($all_value)){
            foreach($all_value as $value){
              $edit_link = site_url('Events/edit_video?id=');
              if(!empty($value->m_video_url)){
                $video_link = $value->m_video_url;
              }
              else{
                $video_link = base_url('uploads/blank');
              }
              if(!empty($value->m_video_image)){
                $image_link = base_url('uploads/video/').$value->m_video_image;
              }
              else{
            ...
            <tr>
              ...
              <td title="Document Type"><?php echo $value->m_video_title; ?></td>
              <td title="PDF"><a href="<?php echo $video_link;?>" class="btn btn-info" target="_blank"><i class="fa fa-file-video-o"></i> Video</a></td>
              <td title="PDF"><a href="<?php echo $image_link;?>" class="btn btn-info" target="_blank"><i class="fa fa-picture-o"></i> Image</a></td>
              <td title="Date"><?php echo date('d-m-Y',strtotime($value->m_video_added));?></td>
              <td title="" style="white-space: nowrap;">
                <a href="<?php echo $edit_link.$value->m_video_id ; ?>" class="btn btn-success btn-action" title="Edit" data-toggle="tooltip"><i class="fa fa-pencil"></i></a> 
                <button class="btn btn-danger btn-action delete-data" data-value="<?php echo $value->m_video_id ; ?>" title="Delete" data-toggle="tooltip"><i class="fa fa-trash"></i></button>
              </td>
            </tr>
            <?php
            $i++;
...
<!-- ========================Script========================== -->
<?php $this->view('js/videos_js');  $this->view('js/custom_js'); ?>
<!-- ========================Script========================== -->