...
//============================Blog============================//
$("form#frm-add-magazines").submit(function(e) { e.preventDefault();
  var clkbtn = $("#btn-add-magazines"); clkbtn.prop('disabled',true);

  var formData = new FormData(this);
  $.ajax({
    type: "POST",
    url: "<?php echo site_url('Magazines/insert_magazines'); ?>",
    data: formData,
    processData: false,
    contentType: false,
    dataType: "JSON", 
    success: function(data) {
      if(data.status=='success'){
        ...
        setTimeout(function(){ window.location = "<?php echo site_url('Magazines'); ?>"; },1000);
...
$("form#frm-update-magazines").submit(function(e) { e.preventDefault();
  var clkbtn = $("#btn-update-magazines"); clkbtn.prop('disabled',true);

  var formData = new FormData(this);
  $.ajax({
    type: "POST",
    url: "<?php echo site_url('Magazines/update_magazines'); ?>",
    data: formData,
    processData: false,
    contentType: false,
    dataType: "JSON", 
...
$("#magazines_tbl").on("click",".delete-magazines",function(){ 
  ...
  swal({
    ...
    if (willDelete) {
        ...
        $.ajax({
          type: "POST",
          url: "<?php echo site_url('Magazines/delete_magazines'); ?>",
...
//============================Blog============================//
$("#magazines_tbl").on('click', '.change-status', function() {
  change_status($(this), "<?php echo site_url('Magazines/change_status'); ?>");
});
