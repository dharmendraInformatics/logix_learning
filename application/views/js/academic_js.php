<script type="text/javascript"> $(document).ready(function(e) {
  $("form#frm-add").submit(function(e) { e.preventDefault();
    ...
    $.ajax({
      type: "POST",
      url: "<?php echo site_url('Academic/insert_academic'); ?>",
      data: formData,
      processData: false,
      contentType: false,
      dataType: "JSON", 
      success: function(data) {
        if(data.status=='success'){
          ...
          setTimeout(function(){
            window.location = "<?php echo site_url('Academic'); ?>"; 
          },1000);
  ...
  $("form#frm-update").submit(function(e) { e.preventDefault();
    ...
    $.ajax({
      type: "POST",
      url: "<?php echo site_url('Academic/update_academic'); ?>",
      data: formData,
      processData: false,
      contentType: false,
      dataType: "JSON", 
  ...
  swal({
    ...
    if (willDelete) {
        ...
        $.ajax({
          type: "POST",
          url: "<?php echo site_url('Academic/delete_Academic'); ?>",
...
<script>
  function viewDetails(id){
    $("#academic_desc").val('');
    var desc = $("#m_academic_desc"+id).val();
    $("#academic_desc").val(desc);
    $('#mymodal').modal('show');
  }
</script>
