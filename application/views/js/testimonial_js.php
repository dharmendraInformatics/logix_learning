...
//=========================Testimonial========================//
$("form#frm-add-testimonial").submit(function(e) { e.preventDefault();
  var clkbtn = $("#btn-add-testimonial"); clkbtn.prop('disabled',true);

  var formData = new FormData(this);
  $.ajax({
    type: "POST",
    url: "<?php echo site_url('Testimonial/insert_testimonial'); ?>",
    data: formData,
    processData: false,
    contentType: false,
    dataType: "JSON", 
    success: function(data) {
      if(data.status=='success'){
        ...
        setTimeout(function(){ window.location = "<?php echo site_url('Testimonial'); ?>"; },1000);
...
$("form#frm-update-testimonial").submit(function(e) { e.preventDefault();
  var clkbtn = $("#btn-update-testimonial"); clkbtn.prop('disabled',true);

  var formData = new FormData(this);
  $.ajax({
    type: "POST",
    url: "<?php echo site_url('Testimonial/update_testimonial'); ?>",
    data: formData,
    processData: false,
    contentType: false,
    dataType: "JSON", 
...
$("#testimonial_tbl").on("click",".delete-testimonial",function(){ 
  ...
  swal({
    ...
    if (willDelete) {
        ...
        $.ajax({
          type: "POST",
          url: "<?php echo site_url('Testimonial/delete_testimonial'); ?>",
...
//=========================Testimonial========================//
$("#testimonial_tbl").on('click', '.change-status', function() {
  change_status($(this), "<?php echo site_url('Testimonial/change_testimonial_status'); ?>");
});

$('#testimonial_tbl').on("click", ".show-desc-btn", function(){
  show_description($(this));
});

$('#testimonial_tbl').on("click", ".show-file-btn", function(){
  var file_link  = "<?php echo base_url('../uploads/testimonials/'); ?>",
...
function show_description(clkbtn){ clkbtn.prop('disabled',true);
  var ai_id = clkbtn.parent('td').data('id'), title = clkbtn.parent('td').attr('title'),
  desc = clkbtn.parent('td').data('desc');
  $("#descModal .modal-title").html(title);

  if(desc != ''){
    $("#descModal #descBody").html(desc);
    $("#descModal").modal('show');
  }else{ swal("Description Not Found.", {icon: "error", timer: 2000, }); }
